...
# --- Nowa klasa dla okna debugowania blobów ---
class BlobDebugWindow(tk.Toplevel):
    def __init__(self, master, **kwargs):
        ...
        # Przechowuj referencje do obrazów, aby uniknąć "garbage collection"
...
# --- Główna klasa aplikacji (połączenie Blobbin i Skalowanie) ---
class CombinedApp:
    def __init__(self, root):
        ...
        self.cap = None
        self.DISPLAY_WIDTH = 960  # Zwiększony rozmiar głównego feeda
        self.display_height = 0
        self.blob_filters = []
        self.is_picking_color = False
        self.current_frame = None  # To będzie skalibrowana i odwrócona klatka
        ...
        # --- Uruchomienie pętli ---
        self.load_settings()  # Załaduj ustawienia blobów
    ...
    def initialize_camera(self):
        ...
        if not self.cap.isOpened():
            print(f"BŁĄD: Nie można otworzyć kamery.")
            return False
    ...
    def create_gui(self):
        ...
        # --- Prawa kolumna (Kontrolki Blobów) ---
    ...
    def update_frame(self):
        try:
            ...
            # `process_blobs` zwraca teraz (maskę, obraz z ramkami blobów)
            # `annotated_image` będzie modyfikowana dalej przez YOLO
            ...
            # Używamy `annotated_image` jako wejścia, aby YOLO rysowało NA ramkach blobów
            results = self.yolo_model(annotated_image, verbose=False)

            # Ręczne rysowanie na klatce (zamiast results[0].plot())
            ...
            for box in results[0].boxes:
                ...
                # Obliczenia i rysowanie pomiarów
                pixel_width = x2 - x1
                pixel_height = y2 - y1
                real_width_mm = 0.0
                real_height_mm = 0.0
                ...
                # Logika pozycjonowania tekstu pomiarów
                text_y_width = y2 + 25
                text_y_height = y2 + 50
            ...
            #
            # <<< USUNIĘTE LINIE cv2.putText DLA DYSTANSU I STOSUNKU >>>
            #
            # Informacje te są teraz wyświetlane tylko w etykietach Tkinter
            ...
            # Przygotowanie obrazu do wyświetlenia w głównym oknie
            ...
            self.video_label.image = photo_orig  # Zachowaj referencję

            # --- 7. Aktualizacja okna DEBUG (jeśli otwarte) ---
            if self.debug_window and self.debug_window.winfo_exists():
                # Przygotuj klatki dla okna debug (muszą mieć ten sam rozmiar)
                debug_w = 480
                ...
                debug_ann_resized = cv2.resize(annotated_image, (debug_w, debug_h))  # Użyj pełnej klatki
    ...
    def process_blobs(self):
        ...
        if not self.blob_filters or self.current_hsv is None:
            zeros = np.zeros_like(self.current_frame[:, :, 0])  # Maska 1-kanałowa
        ...
        for blob in self.blob_filters:
            ...
            if h_l > h_h:  # Przejście przez 0 na skali Hue
            ...
            # Rysowanie konturów dla tego bloba
    ...
    def on_color_pick(self, event):
        ...
        # Przeskaluj kliknięcie z `self.video_label` (DISPLAY_WIDTH x display_height)
    ...
    def add_new_blob_filter(self, h, s, v):
        ...
        # Użyj `self.blob_controls_frame` (wewnątrz Canvas)
    ...
    def create_slider(self, parent_frame, text, var, from_, to_):
        ...
        scale = tk.Scale(frame, variable=var, from_=from_, to=to_, orient=tk.HORIZONTAL, length=200)  # Skrócony suwak
    ...
    def open_settings_window(self):
        ...
        # (Reszta kodu dla okna ustawień jest identyczna jak w blobytest)
